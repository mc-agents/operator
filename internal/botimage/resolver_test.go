...
func TestResolve(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := resolver.Resolve(&tc.spec, nil)
			if err != nil {
...
func TestResolveTakesTheProfileBeforeTheFlags(t *testing.T) {
	...
	azalea, err := resolver.Resolve(&v1alpha1.MinecraftBotSpec{Kind: v1alpha1.BotKindAzalea, MinecraftVersion: "26.1.2"}, profile)
	if err != nil {
		t.Fatalf("Resolve: %v", err)
	}
	if want := "registry.local/mirror/bot-azalea:0.16.0-mc26.1.2"; azalea != want {
		t.Fatalf("azalea got %q, want %q", azalea, want)
	}

	fabric, err := resolver.Resolve(&v1alpha1.MinecraftBotSpec{Kind: v1alpha1.BotKindFabric, MinecraftVersion: "26.1.2"}, profile)
	if err != nil {
		t.Fatalf("Resolve: %v", err)
	}
	if want := "registry.local/mirror/bot-fabric:0.2.0-mc26.1.2"; fabric != want {
		t.Fatalf("a profile without a fabric tag should fall back to the flag: got %q, want %q", fabric, want)
	}
}

func TestResolveRejectsUnknownKind(t *testing.T) {
	resolver := botimage.NewResolver("", botimage.Tags{Fabric: "1"})
	if _, err := resolver.Resolve(&v1alpha1.MinecraftBotSpec{Kind: "quake"}, nil); err == nil {
...
func TestResolveRejectsFabricWithoutVersion(t *testing.T) {
	resolver := botimage.NewResolver("", botimage.Tags{Fabric: "1"})
	_, err := resolver.Resolve(&v1alpha1.MinecraftBotSpec{Kind: v1alpha1.BotKindFabric}, nil)
	if err == nil {
